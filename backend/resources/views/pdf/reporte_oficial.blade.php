...
<head>
    <meta charset="UTF-8">
    <title>{{ $titulo ?? 'Reporte Oficial EIE' }}</title>
    <style>
        @page {
            margin: 1.2cm 1.8cm;
        }
        ...
        <tbody>
            ...
            @foreach ($datos ?? [] as $item)
                <tr>
                    <td>{{ $idx++ }}</td>
                    <td>{{ $item['ci'] ?? 'N/A' }}</td>
                    <td>{{ strtoupper($item['grado'] ?? 'SR') }}</td>
                    <td class="left"><strong>{{ mb_strtoupper(trim(($item['apellidos'] ?? '') . ' ' . ($item['nombres'] ?? '')), 'UTF-8') }}</strong></td>
                    <td>{{ $item['celular'] ?? '—' }}</td>
                    <td>{{ strtoupper($item['estado'] ?? 'ACTIVO') }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
